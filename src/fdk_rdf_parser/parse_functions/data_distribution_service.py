...
from fdk_rdf_parser.rdf_utils import objectValue, resourceList, valueTranslations, dcatApiURI

from .skos_concept import extractSkosConcept

def extractDataDistributionServices(graph: Graph, subject: URIRef) -> List[DataDistributionService]:
    values = []
    for resource in resourceList(graph, subject, dcatApiURI('accessService')):
        ...
        values.append(DataDistributionService(
            uri = resourceUri,
            title = valueTranslations(graph, resource, DCTERMS.title),
            description = valueTranslations(graph, resource, DCTERMS.description),
            endpointDescription = extractSkosConcept(graph, resource, dcatApiURI('endpointDescription'))
        ))

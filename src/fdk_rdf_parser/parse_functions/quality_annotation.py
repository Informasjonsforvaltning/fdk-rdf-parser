...
from fdk_rdf_parser.rdf_utils import objectValue, resourceList, valueTranslations, dqvURI, provURI

def extractQualityAnnotation(graph: Graph, subject: URIRef) -> Dict[str, QualityAnnotation]:
    annotations = {}
    ...
    for annotation in resourceList(graph, subject, dqvURI('hasQualityAnnotation')):
        ...
        annotations[inDimensionValue] = QualityAnnotation(
            inDimension = inDimensionValue,
            hasBody = extractHasBody(graph, annotation)
        )
